...
normalized_cities = {unidecode(city).lower(): city for city in original_cities}

class MessageRequest(BaseModel):
    city: str
    fuzzy_threshold: int = 80

def find_best_match(city_name: str, threshold: int = 80) -> tuple:
    normalized_input = unidecode(city_name).lower()
    
    if normalized_input in normalized_cities:
        return (normalized_cities[normalized_input], 100)
    ...
    if match:
        return (normalized_cities[match], score)
    return (None, 0)
...
async def verify_city(message_request: MessageRequest):
    ...
    threshold = message_request.fuzzy_threshold
    
    matched_city, score = find_best_match(city_name, threshold)
    
    if matched_city:
        if score == 100:
            return {
                "status": "success",
                "message": f"A cidade {matched_city} está na lista!",
                "matched_city": matched_city
            }
        else:
            return {
                "status": "partial",
                "message": f"Você quis dizer {matched_city}? (Similaridade: {score}%)",
                "matched_city": matched_city,
                "similarity": score,
        ...
        return {
            "status": "failure",
            "message": f"Nenhuma correspondência próxima encontrada para '{city_name}'",
            "suggestions": []
        }